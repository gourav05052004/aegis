"""
Honeywell Hackathon Anomaly Detection System - Phase 6: Enterprise SOC Analyst Dashboard
-----------------------------------------------------------------------------------------
...
def load_dashboard_data():
    ...
    df_preds = pd.read_csv(pred_path) if os.path.exists(pred_path) else pd.DataFrame()
    ...
    return df_preds, explanations_dict, cold_start_list, profiles_dict, eval_metrics, df_events

# Load data
df_preds, explanations_dict, cold_start_list, profiles_dict, eval_metrics, df_events = load_dashboard_data()

# -----------------------------------------------------------------------------
# Real-Time Streaming Control (Sidebar)
# -----------------------------------------------------------------------------
st.sidebar.markdown("## 📡 Stream Ingestion Control")
is_streaming = st.sidebar.checkbox("▶️ Start Simulated Real-Time Telemetry Stream", value=False)
    ...
    if os.path.exists(live_json_path):
        try:
            with open(live_json_path, "r") as f:
                live_preds_data = json.load(f)
            if live_preds_data:
                df_preds = pd.DataFrame(live_preds_data)
        except Exception:
            pass
    st.sidebar.markdown("""
        <div class="insight-card" style="border-left-color: #00E676; padding: 12px; margin-top: 10px;">
            <div class="insight-title" style="color: #00E676;">● ACTIVE STREAMING</div>
            <div class="insight-body">Replaying Kafka/EventHub live stream with sub-second scoring (~35ms latency).</div>
        </div>
    """, unsafe_allow_html=True)

# -----------------------------------------------------------------------------
# Global Session State Synchronization for Locked Event ID
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Alert Budget Filter & Executive KPI Metrics
# -----------------------------------------------------------------------------
...
if not df_preds.empty:
    ...
    active_threats_df = df_preds[df_preds["hybrid_risk_score"] >= risk_threshold]
    ...
    with kpi1:
        st.markdown(f"""
            <div class="soc-card" style="padding: 14px;">
                ...
                <div class="kpi-subtitle">7-Day Telemetry Stream</div>
            </div>
        """, unsafe_allow_html=True)
    with kpi2:
        st.markdown(f"""
...
with tab1:
    ...
    if not df_preds.empty:
        # Scatter Plot: Timestamp vs Hybrid Risk Score
        ...
        fig_scatter.add_hline(y=risk_threshold, line_dash="dash", line_color="#FF1744", annotation_text="Alert Threshold")
        ...
        with col_select:
            ...
            # Synchronize session state across all tabs
            st.session_state["locked_event_id"] = selected_event

        # Display Custom HTML Rendered Alert Queue Table with HTML Badges
...
with tab2:
    ...
    if not df_preds.empty and target_event_id in df_preds["event_id"].values:
        ...
        # Extract Entity Chronological Event Sequence from df_events
    ...
    if exp_item:
        ...
        with col_shap_chart:
            ...
            if not df_shap.empty:
                # Uniform High-Contrast Cyber Red Accent Color for all SHAP feature bars
                fig_shap = px.bar(
                    df_shap,
                    x="shap_value",
                    y="feature",
...
# -----------------------------------------------------------------------------
# Real-Time Stream Auto-Refresh Loop
# -----------------------------------------------------------------------------
if is_streaming:
